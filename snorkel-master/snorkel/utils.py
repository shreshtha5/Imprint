...
def matrix_tp1(L, labels):
    return np.ravel([
        np.sum(np.ravel((L[:, j] == 1).todense()) * (labels == 1)) for j in range(L.shape[1])
    ])

def matrix_fp1(L, labels):
    return np.ravel([
        np.sum(np.ravel((L[:, j] == 1).todense()) * (labels == -1)) for j in range(L.shape[1])
    ])

def matrix_tn1(L, labels):
    return np.ravel([
        np.sum(np.ravel((L[:, j] == -1).todense()) * (labels == -1)) for j in range(L.shape[1])
    ])

def matrix_fn1(L, labels):
    return np.ravel([
        np.sum(np.ravel((L[:, j] == -1).todense()) * (labels == 1)) for j in range(L.shape[1])
    ])
...
def matrix_tp(L, labels):
    return np.ravel([
        np.sum(np.dot(np.ones(np.ravel((L[:, j] == 1).todense()))) * np.dot(np.ones((labels == 1)))) for j in range(L.shape[1])
